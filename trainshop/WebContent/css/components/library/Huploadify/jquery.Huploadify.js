(function($) {
	$.fn.Huploadify = function(opts) {
		var itemTemp = '<div id="${fileID}" class="uploadify-queue-item">' +
				...
				'</div>' +
				'<span class="up_filename" style="display:block;float:left;margin-top:6px;width:45%;">${fileName}</span>' +
				'<span class="delfilebtn btn btn-warning cancel"><i class="fa fa-times"></i> 删除</span></div>'
				//'<span class="uploadbtn">上传</span>' +
				;
		var defaults = {
			fileTypeExts : '',// 允许上传的文件类型，格式'*.jpg;*.doc'
			uploader : '',// 文件提交的地址
			getUploadedFilePath : '', //获取已上传文件路径
			auto : false,// 是否开启自动上传
			...
			buttonText : '添加文件',// 上传按钮上的文字
			removeTimeout : 10,// 上传完成后进度条的消失时间
		...
		this.each(function() {
			...
			var inputStr = '<input id="select_btn_'
					+ instanceNumber
					+ '" class="selectbtn" style="display:none;" type="file" name="files"';
			inputStr += option.multi ? ' multiple' : '';
			inputStr += ' accept="';
			inputStr += getAcceptString(option.fileTypeExts);
			inputStr += '"/>';
			inputStr += '<a id="file_upload_'
					+ instanceNumber
					+ '-button" href="javascript:void(0)" class="uploadify-button btn btn-success"><i class="glyphicon glyphicon-plus"></i>';
			inputStr += option.buttonText;
			inputStr += '</a>';
			//inputStr += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="#uploadFileListModel" class="btn btn-xs btn-success" data-toggle="modal">已有附件<span class="badge">4</span></a>';
			var uploadFileListStr = '<div id="file_upload_' + instanceNumber
			...
			fileObj = {
				...
				getUploadedFileurl : option.getUploadedFilePath, // ajax地址
				fileFilter : [], // 过滤后的文件数组
					...
					if (typeArray.length > 0) {
						for (var i = 0, len = files.length; i < len; i++) {
							...
							if (parseInt(formatFileSize(thisFile.size, true)) > option.fileSizeLimit) {
								showMessage('文件' + thisFile.name + '大小超出限制！');
								continue;
							}
							if ($.inArray(thisFile.name.split('.').pop().toLowerCase(), typeArray) >= 0) {
								if(this.funCheckFileExist(thisFile.name)){
									showMessage('文件' + thisFile.name + '已经存在！');
								}
								else{
									arr.push(thisFile);
								}
							} else {
								showMessage('文件' + thisFile.name + '类型不允许！');
							}
						}
					}
					return arr;
				},
				funCheckFileExist : function(fileName){
					var isExtist = false;
					for (var i = 0; i < this.fileFilter.length; i++){
						if(this.fileFilter[i].name == fileName){
							isExtist = true;
						}
					}
					var uploadedFileList = this.funGetUploadFiles();
					for (var i = 0; i < uploadedFileList.length; i++){
						if(uploadedFileList[i].S_FILENAME == fileName){
							isExtist = true;
						}
					}
					return isExtist;
...
//							subFileName = file.name.substring(0, 20) + "..."
//						}
//						else{
//							subFileName = file.name;
//						}
						var $html = $(option.itemTemplate.replace(
								/\${fileID}/g,
								'fileupload_' + instanceNumber + '_'
										+ file.index).replace(/\${fileName}/g,
								file.name).replace(/\${fileSize}/g,
								formatFileSize(file.size)).replace(
								/\${instanceID}/g, _this.attr('id')));
						// 如果是自动上传，去掉上传按钮
						if (option.auto) {
						...
						if (option.breakPoints) {
							...
							initUppercent = (uploadedSize / file.size * 100)
									.toFixed(2)
									+ '%';

							$html.find('.uploadify-progress-bar').css('width',
									initWidth);
						}
						...
						if (option.showUploadedSize) {
							var num = '<span class="progressnum"><span class="uploadedsize">'
									+ initFileSize
									+ '</span>/<span class="totalsize">${fileSize}</span></span>'
											.replace(/\${fileSize}/g,
						...
						if (option.showUploadedPercent) {
							var percentText = '<span class="up_percent">'
									+ initUppercent + '</span>';
							$html.find('.uploadify-progress')
									.after(percentText);
						}
				...
				onProgress : function(file, loaded, total) {
					var eleProgress = _this.find('#fileupload_'
							+ instanceNumber + '_' + file.index
							+ ' .uploadify-progress');
					...
					var progressBar = eleProgress
							.children('.uploadify-progress-bar');
					var oldWidth = option.breakPoints ? parseFloat(progressBar
							.get(0).style.width
							|| 0) : 0;
					...
					if (option.showUploadedSize) {
						eleProgress.nextAll('.progressnum .uploadedsize')
								.text(formatFileSize(loaded));
						eleProgress.nextAll('.progressnum .totalsize')
				...
				funGetProgressWidth : function(index) {
					var eleProgressBar = _this.find('#fileupload_'
							+ instanceNumber + '_' + index
				...
				funGetUploadedSize : function(file) {
					if (option.getUploadedSize) {
						...
						if (option.saveInfoLocal) {
							return parseInt(localStorage.getItem(file.name))
									|| 0;
						}
					}
				},
				...
				funDealFiles : function(files) {
					var fileCount = _this
							.find('.uploadify-queue .uploadify-queue-item').length;// 队列中已经有的文件个数
					for (var i = 0, len = files.length; i < len; i++) {
						files[i].index = ++fileCount;
						files[i].id = 'fileupload_' + instanceNumber + '_'
								+ files[i].index;
					}
					// 执行选择回调
				...
				funDeleteFile : function(index) {
					for (var i = 0, len = this.fileFilter.length; i < len; i++) {
						...
						if (file.index == index) {
							...
							_this.find('#fileupload_' + instanceNumber + '_'
									+ index).fadeOut();
							_this.find('#fileupload_' + instanceNumber + '_'
				...
				//获取已上传文件
				funGetUploadFiles : function(){
					var uploadedFileList = new Array();
				if(option.formData.businessId != undefined && option.formData.businessId != "-1"){
						$.ajax({
							type : 'post',
				            //contentType : 'application/x-www-form-urlencoded',  
				            url : this.getUploadedFileurl,
				            data: 'businessType=' + option.formData.businessType + '&businessId=' + option.formData.businessId, 
				            dataType : 'json',  
				            //同步  
				            async : false,  
				            success : function(data) {
				            	if(data.DATA != undefined){
					                if(data.DATA.item != undefined){
										var resultData = data.DATA.item;
										if(resultData instanceof Array){
										 	for( var i = 0; i < resultData.length; i++){
										 		uploadedFileList.push(resultData[i]);
											}
										}
										else{
											uploadedFileList.push(resultData);
										}
										
									}
				            	}
								
								var temp = $("#showMoreUploadFileBtnId");
								if(temp.length <= 0){
									$('.uploadify-button').after('&nbsp;&nbsp;<button type="button"  id="showMoreUploadFileBtnId" href="#uploadFileListModel" class="btn btn-success fileinput-button" data-toggle="modal">已有附件<span id="uploadFileListLength" class="badge">' + uploadedFileList.length + '</span></button>');
								}
								else{
									$("#uploadFileListLength").text(uploadedFileList.length)
								}
				...
				funUploadFile : function(file) {
					...
					var thisfile = _this.find('#fileupload_' + instanceNumber
							+ '_' + file.index);
					var regulateView = function() {
						if (fileObj.uploadOver) {
							thisfile.find('.uploadify-progress-bar').css(
									'width', '100%');
							option.showUploadedSize
									&& thisfile.find('.uploadedsize')
											.text(thisfile.find('.totalsize')
													.text());
							option.showUploadedPercent
									&& thisfile.find('.up_percent')
											.text('100%');
							
							
							var temp = $("#showMoreUploadFileBtnId");
							if(temp.length <= 0){
								$('.uploadify-button').after('&nbsp;&nbsp;<button type="button" id="showMoreUploadFileBtnId" href="#uploadFileListModel" class="btn btn-success fileinput-button" data-toggle="modal">已有附件<span id="uploadFileListLength" class="badge">' + 0 + '</span></button>');
							}
							var fileLength = Number($("#uploadFileListLength").text());
							$("#uploadFileListLength").text(fileLength + 1);
						}
					}// 校正进度条和上传比例的误差

					try {
						xhr = new XMLHttpRequest();// 尝试创建 XMLHttpRequest 对象，除
													// IE 外的浏览器都支持这个方法。
					} catch (e) {
						xhr = ActiveXobject("Msxml12.XMLHTTP");// 使用较新版本的 IE 创建
																// IE
					...
					if (option.breakPoints) {
						...
						var uploadedSize = parseInt(this
								.funGetUploadedSize(originalFile));
						// 对文件进行切割，并保留原来的信息
						file = originalFile.slice(uploadedSize, uploadedSize
										+ option.fileSplitSize);
						file.name = fileName;
						file.id = fileId;
					...
					if (xhr.upload && uploadedSize !== false) {
						...
						xhr.upload.addEventListener("progress", function(e) {
									fileObj.onProgress(file, e.loaded,
											originalFile.size);
								}, false);
						...
						xhr.onreadystatechange = function(e) {
							if (xhr.readyState == 4) {
								...
								if (fileObj.uploadOver) {
									...
									//fileObj.fileFilter = [];
									//fileObj.uploadFileList = $(".uploadify-queue");
								};
						...
						// option.formData['last_time'] =
						// originalFile.lastModifiedDate.getTime();
		...
		return {
			...
			upload : function(fileIndex, message) {
				if (fileIndex === '*') {
			...
			getFiles : function(){
				return fileObj.funGetUploadFiles();
			},
			...
			disable : function(instanceID) {
				var parent = instanceID ? $('file_upload_' + instanceID
						+ '-button') : $('body');
				parent.find('.uploadify-button')
						.css('background-color', '#888').off('click');
			},
			ennable : function(instanceID) {
				//点击上传按钮时触发file的click事件
				var parent = instanceID ? $('file_upload_' + instanceID
						+ '-button') : $('body');
				parent.find('.uploadify-button').css('background-color',
						'#707070').on('click', function() {
